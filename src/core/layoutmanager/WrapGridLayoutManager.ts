...
export class WrapGridLayoutManager extends LayoutManager {
  ...
  public relayoutFromIndex(startIndex: number, itemCount: number): void {
    let isForceComputedLayout = false;
    if (this._isHorizontal) {
      isForceComputedLayout = this._totalWidth <= this._window.width;
    } else {
      isForceComputedLayout = this._totalHeight <= this._window.height;
    }
    // 定位第一个相邻的索引
  ...
  private _setFinalDimensions(maxBound: number): void {
    ...
    this.layoutListener?.onLayoutChange({
